...
if uploaded_file is not None:
    ...
    if not df.isnull().any().any():
        ...
        st.write("There are still missing values. Repeat the process to handle them.")

    # New Step 3: Fill Missing Values in Categorical Columns
    ...
    if categorical_columns_with_missing:
        ...
        # Allow users to select multiple categorical columns to handle missing values
        selected_columns = st.multiselect(
            "Select categorical columns to handle missing values (you can choose multiple columns):",
            options=categorical_columns_with_missing,
            key='select_categorical_columns'
        )
        ...
        if selected_columns:
            for selected_column in selected_columns:
                fill_option = st.radio(
                    ...
                    key=f'radio_fill_option_{counter}'  # Unique key for each column
                )
                ...
                if fill_option == "Fill with Mode":
                    ...
                    st.write(f"Retained missing values in **{selected_column}** as is.")
        ...
        if not categorical_columns_with_missing:
            ...
            st.write("There are still missing values in the following categorical columns:", categorical_columns_with_missing)
    else:
        st.write("No categorical columns with missing values were found!")
